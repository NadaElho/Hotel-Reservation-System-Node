...
class ReservationController {
  ...
  schedule() {
    cron.schedule("0 0 * * *", async () => {
      ...
      allResevations.forEach(async (reservation) => {
        if (
          todayDate >= reservation.checkOut &&
          reservation.status == "663a8186e3427acea0ef0b56"
        ) {
            ...
            { _id: reservation.id },
            {
              $set: {
                status: reservation.paid
                  ? "663a81a4e3427acea0ef0b58"
                  : "663a8158e3427acea0ef0b54",
              },
            }
          );
  ...
  async addNewReservation(body) {
    ...
    const room = await Room.findOne({ _id: body.roomId }).populate(
      "promotionId"
    );
    const user = await User.findOne({ _id: userId }).populate("subscriptionId");
    let calcTotalPrice = 0;

    if (room?.promotionId.percentage) {
      calcTotalPrice =
        calcNoOfNights * room.price * (1 - room?.promotionId.percentage / 100);
    } else {
